...
color_table_plot = function(data, title, subtitle=NULL, digits=0) {
  ggplot(data, aes(x=stemintentie, y=name, label=round(value, digits = digits), fill=value)) +
    geom_tile()+ geom_text()+
...
print(color_table_plot(d, "Stemintentie en mediagebruik", digits=1))
d %>% pivot_wider(names_from="stemintentie", names_sort=T) %>% 
  export_data("Wave0: Stemintentie en mediagebruik", label=T) %>% cat()
```

```{r wave0-media-party-specific}
pct_variables = c(I2="Kranten", I3="TV", I4="Online", I5="Social", I7="Apps")
for (var in names(pct_variables)) {
  label = paste("Stemintentie en", pct_variables[[var]])
  message(paste(var, label))
  d = long_vote %>% filter(question==var, name != "Other, namely:") %>% create_table(compute_perc = T)
  #data_export[[label]] = d %>% pivot_wider(names_from="stemintentie", names_sort=T) 
  print(color_table_plot(data=d, title = label, subtitle = "(Percentage van stemmers voor die party; meerdere keuzes mogelijk)"))
  }
```
...
#data_export[["Vertrouwen in Media"]] = d %>% pivot_wider(names_from="stemintentie", names_sort=T) 
color_table_plot(d, "Vertrouwen in Media", subtitle="(Cijfer van 1=laag tot 10=hoog)", digits=1)
```

# Trust in Institutions

```{r wave0-trust-institution}
d = long_vote %>% filter(question=="F2") %>% create_table()
#data_export[["Vertrouwen in Instituties"]] = d %>% pivot_wider(names_from="stemintentie", names_sort=T) 
color_table_plot(d, "Vertrouwen in Instituties", subtitle="(Cijfer van 1=laag tot 10=hoog)", digits=1)
```
...
#data_export[["Vertrouwen gegroepeerd"]] = d %>% pivot_wider(names_from="stemintentie", names_sort=T) 
color_table_plot(d, "Vertrouwen in Instituties (gegroepeerd)", 
                 subtitle="(Cijfer van 1=laag tot 10=hoog)", digits=1)
```

## Negativo's vs Positivo's

If we categorize people by sufficient trust in democracy (>5.5) or not, what do we get?
...
color_table_plot(dt, "Welk cijfer krijgt de democratie?", subtitle="(Percentage van stemmers op een partij)")
#data_export[["Rapportcijfer per partij"]] = dt
```
