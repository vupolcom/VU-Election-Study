...
d2 %>% ggplot(aes(x=wave, y=name, fill=fillcolor, label=round(value,1))) + 
  ...
  scale_fill_gradient2(midpoint=5.5, low="red", high="blue") + geom_text() + 
...
d3 %>% filter(variable=="B2") %>% 
  ggplot(aes(x=wave, y=name, fill=value, label=round(value,1))) + geom_tile() + facet_grid(cols=vars(rile)) + theme_nrc() + 
  geom_text() + scale_fill_gradient(low="white", high="blue", guide = F) + 
...
d3 %>% filter(variable=="B3") %>% 
  ggplot(aes(x=wave, y=name, fill=value, label=round(value,1))) + geom_tile() + facet_grid(cols=vars(rile)) + theme_nrc() + 
  geom_text() + scale_fill_gradient2(low="red", mid="white", high="green", midpoint = 5.5, guide = F) + 
...
Interestingly, evaluation of their media performance does differ: rightist voters are overall more critical of rightist parties (especially FvD, PVV), while leftist voters are a bit more critical of GL. 
...
Of the undecided voters at the start, only `r perc(1-still_undecided)` is still undecided.
...
```{r undecided-demo}
scale = function(x) scales::rescale(as.numeric(x))
r2 = resp %>% mutate(education=1-scale(education), 
         age=scale(age), 
         gender=scale(gender),
...
ggplot(d, aes(x=wave, y=measure, fill=value, label=round(value*100))) +
  ...
  ggtitle("Demographics of undecided voters per wave", 
          "(rescaled to 0-100, gender is percentage female, leftright is 0=left, 100=right)")
```
...
Interestingly, even though this group has a lower overall education, they score better on political knowledge, probably as a result of being older. 
