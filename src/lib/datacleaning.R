...
load_survey = function(survey_id) {
  ...
  qualtrics_api_credentials(api_key = API,  base_url = "vuass.fra1.qualtrics.com")
...
clean_A2 = function(d) {
  A = d %>%
    select(iisID, A1:A2, A2_followup1,A2_followup2,
           matches("A[23]_DO_\\d+"), A4) %>%
    mutate(A1 = recode(A1,
                       `2` = 0,
                       `3` = 998,
                       `4` = 999),
           A2 = recode(A2,
                       `16` = 999),
           A2_text = case_when(!is.na(`A2_followup1`) ~ `A2_followup1`,
                               !is.na(`A2_followup2`) ~ `A2_followup1`),
           A2_text = if_else(A1==1, A2_text, "NA")) %>%
    unite(order_A2, matches("A2_DO_\\d+"), sep="|") %>%
    unite(order_A3, matches("A3_DO_\\d+"), sep="|") 
  ...
  left_join(A, A3, by = "iisID") %>%
    select(iisID, A1:A2,A2_text, order_A2, A3, order_A3, matches("A3_\\d+"), A4) 
}
...
clean_C = function(d) {
  ...
  C12 = 
    d %>% select(iisID, matches("\\d+_C[12]_[12]_[234]$")) %>% 
    pivot_longer(-iisID) %>% 
    ...
    filter(!is.na(value)) %>%
    distinct(iisID,party,variable, choice,screen, .keep_all=TRUE) %>%
  ...
  C1_TEXT = d %>% select(iisID, matches("\\d+_C1_[12]_[234]_38_TEXT$")) %>% 
    pivot_longer(-iisID) %>% 
...
clean_F = function(d) {
  d %>% select(iisID, matches("^F[12]")) %>% rename(F2_11=F2_15) %>% 
    unite(order_F1, matches("F1_DO"), sep="|") %>% unite(order_F2, matches("F2_DO"), sep="|")
}
