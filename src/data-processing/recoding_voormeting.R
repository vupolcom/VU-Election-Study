...
M = clean_meta(d0) %>% 
  add_column(wave="pre-wave", .after = 'iisID')
A = clean_A(d0)
B = clean_B(d0)
C = clean_C(d0)
F = clean_F(d0)
...
D <- d0 %>%
  select(iisID, D1_1, D1_2, D1_3, D1_4,
         ...
         matches("D2_timer"),
         D3_1, D3_2, D3_3, D3_4, 
         matches("D3_timer"), progress = Progress) %>%
  mutate(D1_1 = ifelse(D1_1 == 6, 1, 0),
         ...
         D = D1_sum + D2_sum + D3_sum,
         D_time_secs = D1_time + D2_time + D3_time,
         D_clicks = D1_timer_Click.Count + D2_timer_Click.Count +
           D3_timer_Click.Count) %>%
  mutate(check = ifelse(duplicated(iisID) & progress <= 100, 1, 0)) %>%
  filter(check == 0) %>% 
  select(iisID, D, D_time_secs, D_clicks) 
...
E <- d0 %>%
  ...
  mutate(E1 = recode(E1,
                     ...
                     `15` = 13, `16` = 14, `17` = 997, `18` = 998,
                      `19` = 999),
         check = ifelse(duplicated(iisID) & progress <= 100, 1, 0)) %>%
  unite(order_E1, matches("E1_DO"), sep="|") %>%
  filter(check == 0) %>% 
...
H <- d0 %>%
  mutate(H4 = recode(H4, `8` = 999),
         ...
         check = ifelse(duplicated(iisID) & Progress <= 100, 1, 0)) %>%
  unite(order_H6, matches("H6_DO"), sep="|") %>%
  filter(check == 0) %>% 
  select(iisID, H3:H6_3, order_H6, H7)

# Recode Block I News Consumption
I_trust <- d0 %>% 
  unite(order_I8, I8_DO_1:I8_DO_15, sep="|") %>%
